...
let comparisonTools = [];
function getPrimaryCategory(tool, query) {
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="mb-3">
                                        <ul class="text-sm text-gray-600 space-y-1">
                                            ${tool.features.map(featureKey => `<li class="flex items-start">
                                                    <svg class="w-4 h-4 text-green-500 mr-2 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                                    </svg>
                                                    <span>${NORMALIZED_FEATURES[featureKey] || featureKey}</span>
                                                </li>`).join('')}
                                        </ul>
                                    </div>
                                    <div class="flex flex-wrap gap-2">
                                        ${tool.categories.slice(0, 3).map(cat => `<span class="px-3 py-1 bg-indigo-50 text-indigo-600 rounded-full text-sm">${cat}</span>`).join('')}
                                    </div>
                                </div>
                            </div>
                        </div>
...
function addToComparison(toolName) {
...
function renderComparison() {
    ...
    if (comparisonTools.length === 1) {
        ...
        comparisonContent.innerHTML = `
            <div class="md:col-span-2 p-6 bg-white rounded-xl shadow-lg border-2 border-indigo-200">
                ...
                <div class="mb-4">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ${tool.categories.map(cat => `<span class="px-3 py-1 bg-indigo-50 text-indigo-600 rounded-full text-sm">${cat}</span>`).join('')}
                    </div>
                </div>
                <div>
        ...
        const categorizeFeatures = (currentTool, otherTool) => {
            const common = [];
            const unique = [];
            const missing = [];
                ...
                <div class="p-6 bg-white rounded-xl shadow-lg border-2 border-indigo-200">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex flex-wrap gap-1.5">
                            ${tool.categories.slice(0, 4).map(cat => `<span class="px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-full text-xs">${cat}</span>`).join('')}
                        </div>
                    </div>
                    <div>
                        <h4 class="text-xs font-semibold text-gray-700 mb-2">Features:</h4>
                        <ul class="text-xs space-y-1">
                            ${common.map(featureKey => `<li class="flex items-start text-gray-500">
                                    <svg class="w-3.5 h-3.5 text-gray-400 mr-1.5 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                    </svg>
                                    <span>${NORMALIZED_FEATURES[featureKey] || featureKey}</span>
                                </li>`).join('')}
                            ${unique.map(featureKey => `<li class="flex items-start text-green-700">
                                    <svg class="w-3.5 h-3.5 text-green-500 mr-1.5 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                    </svg>
                                    <span>${NORMALIZED_FEATURES[featureKey] || featureKey}</span>
                                </li>`).join('')}
                            ${missing.map(featureKey => `<li class="flex items-start text-red-600">
